...
class odoosv_user(models.Model):
        ...
        for r in self:
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_consumidor_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_consumidor_id.id, 'refund_tax_id': tax.id})
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_contribuyente_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_contribuyente_id.id, 'refund_tax_id': tax.id})
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_compras_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_iva_compras_id.id, 'refund_tax_id': tax.id})
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_retencion_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_retencion_id.id, 'refund_tax_id': tax.id})
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_perceccion_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_perceccion_id.id, 'refund_tax_id': tax.id})
            ...
            if tax:
                ...
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_isr_id.id, 'invoice_tax_id': tax.id})
                self.env['account.tax.repartition.line'].create(
                    {'factor_percent': 100, 'repartition_type': 'tax', 'account_id': r.account_isr_id.id, 'refund_tax_id': tax.id})
    ...
    def create_fiscal_position(self):
        for r in self:
            ...
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_nosujeto_compra_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_iva_consumidor_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_isr_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_iva_consumidor_id.id})
            ...
            fp.tax_ids.unlink()
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_iva_contribuyente_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_isr_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_iva_contribuyente_id.id})
            ...
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_perceccion_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_iva_contribuyente_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_retencion_id.id})

            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_retencion_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_isr_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_iva_contribuyente_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_perceccion_id.id})
            ...
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_exento_venta_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_iva_compras_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_exento_venta_id.id})
            ...
            fp.tax_ids.unlink()
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_compra.id, 'tax_dest_id': r.tax_exento_compra_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_tangible_venta.id, 'tax_dest_id': r.tax_exento_venta_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_compra.id, 'tax_dest_id': r.tax_exento_compra_id.id})
            self.env['account.fiscal.position.tax'].create(
                {'position_id': fp.id, 'company_id': r.id, 'tax_src_id': r.tax_base_servicio_venta.id, 'tax_dest_id': r.tax_exento_venta_id.id})
    ...
    def create_docs(self):
        for r in self:
            ...
            # factura
            dic = {}
            factura = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Factura'), ('tipo_movimiento', '=', 'out_invoice')], limit=1)
            if not factura:
                dic['name'] = 'Factura'
                dic['company_id'] = r.id
            ...
            # ccf
            dic = {}
            ccf = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'CCF'), ('tipo_movimiento', '=', 'out_invoice')], limit=1)
            if not ccf:
                dic['name'] = 'CCF'
                dic['company_id'] = r.id
                dic['tipo_movimiento'] = 'out_invoice'
...
if not partner.nrc:
    raise ValidationError('El cliente debe tener NRC')
                ...
                self.env['odoosv.fiscal.document'].create(dic)
            dic = {}
            exportacion = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Exportacion'), ('tipo_movimiento', '=', 'out_invoice')], limit=1)
            if not exportacion:
...
if not partner.tipo_localidad=='NoDomiciliado':
    raise ValidationError('El cliente no debe ser local')
            ...
            # Nota de Credito
            dic = {}
            notacredito = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Nota de Credito'), ('tipo_movimiento', '=', 'out_refund')], limit=1)
            if not notacredito:
...
if not partner.nrc:
    raise ValidationError('El cliente debe tener NRC')
            ...
            dic = {}
            notacredito = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Nota de Debito'), ('tipo_movimiento', '=', 'out_invoice')], limit=1)
            if not notacredito:
...
if not partner.nrc:
    raise ValidationError('El cliente debe tener NRC')
                ...
                self.env['odoosv.fiscal.document'].create(dic)
            dic = {}
            devolucion = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Devolucion'), ('tipo_movimiento', '=', 'out_refund')], limit=1)
            if not devolucion:
                dic['name'] = 'Devolucion'
                dic['company_id'] = r.id
                dic['tipo_movimiento'] = 'out_refund'
            ...
            # Recibo
            dic = {}
            recibo = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Recibo'), ('tipo_movimiento', '=', 'in_invoice')], limit=1)
            if not recibo:
                dic['name'] = 'Recibo'
                dic['company_id'] = r.id
...
if partner.contribuyente:
    raise ValidationError('El proveedor no debe ser contribuyente')
            ...
            # ccf
            dic = {}
            ccf = self.env['odoosv.fiscal.document'].search([('company_id', '=', r.id), ('name', '=', 'CCF'), ('tipo_movimiento', '=', 'in_invoice')],
                                                            limit=1)
            if not ccf:
                dic['name'] = 'CCF'
                dic['company_id'] = r.id
...
if not partner.nrc:
    raise ValidationError('El proveedor debe tener NRC')
            ...
            dic = {}
            importacion = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Importacion'), ('tipo_movimiento', '=', 'in_invoice')], limit=1)
            if not importacion:
...
if not partner.tipo_localidad=='NoDomiciliado':
    raise ValidationError('El Proveedor no debe ser local')
            ...
            dic = {}
            notacredito = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Nota de Credito'), ('tipo_movimiento', '=', 'in_refund')], limit=1)
            if not notacredito:
                dic['name'] = 'Nota de Credito'
                dic['contribuyente'] = True
                dic['company_id'] = r.id
...
if not partner.nrc:
    raise ValidationError('El Proveedor debe tener NRC')
            ...
            dic = {}
            notacredito = self.env['odoosv.fiscal.document'].search(
                [('company_id', '=', r.id), ('name', '=', 'Nota de Debito'), ('tipo_movimiento', '=', 'in_invoice')], limit=1)
            if not notacredito:
                dic['name'] = 'Nota de Debito'
                dic['company_id'] = r.id
                dic['tipo_movimiento'] = 'in_invoice'
    ...
    def button_create_landed_costs(self):
        ...
        landed_costs = self.env['stock.landed.cost'].create({
            ...
            'cost_lines': [(0, 0, {
                ...
                'price_unit': l.currency_id._convert((l.price_subtotal if self.move_type == 'in_invoice' else (l.price_subtotal * -1)),
                                                     l.company_currency_id, l.company_id, l.move_id.date),
...
class odoosv_documento(models.Model):
    ...
    tipo_movimiento = fields.Selection(
        selection=[('in_invoice', 'Factura Proveedor'), ('out_invoice', 'Factura Cliente'), ('in_refund', 'Nota Credito Proveedor'),
                   ('out_refund', 'Nota Credito Cliente'), ('entry', 'Entry')], string="Tipo Documento")
...
#    razon_id=fields.Many2one('odoosv.razon_inventario',string="Razón")
...
class odoosv_pago_register(models.TransientModel):
    ...
    def _get_batch_communication(self, batch_result):
        ...
        '''
        labels = set((
                         line.move_id.tipo_documento_id.name + ':' + line.move_id.doc_numero if line.move_id.tipo_documento_id else line.name) or line.move_id.ref or line.move_id.name
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['transferencia'] = {'mode': 'multi', 'domain': [('type', '=', 'bank')]}
        return res
